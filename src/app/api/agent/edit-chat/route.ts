import { NextRequest, NextResponse } from "next/server";
import Anthropic from "@anthropic-ai/sdk";
import type { SavedMemory } from "@/types/agent";
import type { TripData } from "@/types/trip";
import { agentTools } from "@/lib/agent/tools";
import { buildEditSystemPrompt } from "@/lib/agent/edit-system-prompt";
import { handleToolCalls } from "@/lib/agent/tool-handlers";
import { loadMemories } from "@/lib/supabase/memories";

const MAX_TOOL_ROUNDS = 5;

const anthropic = new Anthropic();

interface EditChatRequest {
  messages: { role: "user" | "assistant"; content: string }[];
  tripData: TripData;
  userId: string | null;
  collectField?: string;
}

export async function POST(req: NextRequest) {
  try {
    const body = (await req.json()) as EditChatRequest;
    const { messages, tripData, userId, collectField } = body;

    if (!messages || !Array.isArray(messages) || messages.length === 0) {
      return NextResponse.json(
        { error: "messages array is required" },
        { status: 400 }
      );
    }

    // Load user memories
    let userMemories: SavedMemory[] = [];
    if (userId) {
      userMemories = await loadMemories(userId);
    }

    // Build edit-specific system prompt
    const systemPrompt = buildEditSystemPrompt(tripData, userMemories, collectField);

    const claudeMessages: Anthropic.MessageParam[] = messages.map((m) => ({
      role: m.role === "user" ? "user" : "assistant",
      content: m.content,
    }));

    // Tool-use loop
    let currentTripData: TripData = { ...tripData };
    const allMemories: SavedMemory[] = [];
    let formComplete = false;
    let finalText = "";

    for (let round = 0; round < MAX_TOOL_ROUNDS; round++) {
      const response = await anthropic.messages.create({
        model: "claude-sonnet-4-20250514",
        max_tokens: 1024,
        system: systemPrompt,
        tools: agentTools,
        messages: claudeMessages,
      });

      const textBlocks = response.content.filter(
        (b): b is Anthropic.TextBlock => b.type === "text"
      );
      if (textBlocks.length > 0) {
        finalText = textBlocks.map((b) => b.text).join("\n");
      }

      const toolUseBlocks = response.content.filter(
        (b): b is Anthropic.ToolUseBlock => b.type === "tool_use"
      );

      if (toolUseBlocks.length === 0) break;

      const toolResult = await handleToolCalls(
        toolUseBlocks.map((b) => ({
          id: b.id,
          name: b.name,
          input: b.input as Record<string, unknown>,
        })),
        currentTripData,
        userId
      );

      currentTripData = toolResult.updatedTripData;
      allMemories.push(...toolResult.memories);
      if (toolResult.formComplete) formComplete = true;

      claudeMessages.push({
        role: "assistant",
        content: response.content,
      });

      claudeMessages.push({
        role: "user",
        content: toolUseBlocks.map((b) => ({
          type: "tool_result" as const,
          tool_use_id: b.id,
          content: toolResult.toolResults[b.id] ?? JSON.stringify({ success: true }),
        })),
      });

      if (response.stop_reason === "end_turn") break;
    }

    return NextResponse.json({
      message: finalText || "I've updated your trip details!",
      updatedTripData: currentTripData,
      memories: allMemories,
      formComplete,
    });
  } catch (error) {
    console.error("Edit agent chat error:", error);
    return NextResponse.json(
      { error: "Something went wrong. Please try again." },
      { status: 500 }
    );
  }
}
